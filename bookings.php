    ...
    <div class="row">
      ...
      <?php 
        ...
        $query = "SELECT 
          ...
          WHERE (
              bo.booking_status IN ('pending', 'booked', 'cancelled', 'payment failed')
          ) AND (bo.user_id = ?)
        ...
        while($data = mysqli_fetch_assoc($result))
        {
          ...
          else if($data['booking_status']=='cancelled')
          {
            ...
            if($data['refund']==0){
              ...
              $btn="<span class='badge bg-success'>Refunded!</span><br><br>";
              $btn.="<a href='generate_pdf.php?gen_pdf&id=$data[booking_id]' class='btn btn-dark btn-sm shadow-none'>Download PDF</a>";
            }
          }
          else
          ...
          // $nightsCount = calculateNights($data['check_in'], $data['check_out']);
          ...
          echo <<<bookings
            <div class='col-md-4 px-4 mb-4'>
              <div class='bg-white p-3 rounded shadow-sm'>
                ...
                <p>LKR {$data['price']} / per night</p>
                <p>
                  <b>Check in: </b> $checkin <br>
                  <b>Check out: </b> $checkout
                </p>
                <p>
                  <b>Amount: </b> LKR $amount <br>
                  <b>Order ID: </b> {$data['order_id']} <br>
                  <b>Date: </b> $date  <br>
                  <b>Paid Amount: </b>LKR {$data['total_pay']}
                </p>
          bookings;
          ...
          if (isset($data['meal_plan_name'])) {
            echo <<<bookings
                <p class='mb-0'>
                  <b>Meal Plan: </b> {$data['meal_plan_name']} ({$data['meal_plan_code']})<br>
                  <b>Meal Plan Description: </b> {$data['meal_plan_description']}
                </p>
          bookings;
          }
